...
//GPU卷积操作1
__global__
void convolutionForGPU1(double* input, double* kernel, double* output) {
    int index = blockIdx.x * blockDim.x + threadIdx.x;//当前下标
    int stride = blockDim.x * gridDim.x;//步长

    for (int i = index; i < (INPUT_SIZE - KERNEL_SIZE + 1)/2 ; i+=stride) {
        output[i]=0.0f;
...
//GPU卷积操作2
__global__
void convolutionForGPU2(double* input, double* kernel, double* output) {
    int index = blockIdx.x * blockDim.x + threadIdx.x;//当前下标
    int stride = blockDim.x * gridDim.x;//步长

    for (int i = (INPUT_SIZE - KERNEL_SIZE + 1)/2 + index; i < INPUT_SIZE - KERNEL_SIZE + 1 ; i+=stride) {
        output[i]=0.0f;
        for (int j = 0; j < KERNEL_SIZE; ++j) {
            output[i]+=input[i+j]*kernel[j];
        }
